...
import { Course, Hole } from '@/app/api/courses/route';
  ...
  const fetchCourse = async () => {
    try {
      ...
      setFormData({
        ...
        holes: course.holes || [],
        menTees: (course.menTees || []).map(tee => ({
          name: tee.name,
          cr: String(tee.cr),
          slope: String(tee.slope)
        })),
        womenTees: (course.womenTees || []).map(tee => ({
          name: tee.name,
          cr: String(tee.cr),
          slope: String(tee.slope)
        }))
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        throw new Error(errorData.details || 'Failed to update course');
      }
    ...
    return (
      <main className="p-8">
        <div className="max-w-4xl mx-auto">
          <p className="text-brand-dark">Loading course...</p>
        </div>
      </main>
    );
  }
    ...
    <main className="p-8">
      <div className="max-w-4xl mx-auto">
        <div className="mb-8">
          ...
          <h1 className="text-3xl font-bold text-brand-dark">Edit Course</h1>
        </div>
